...
>A: 两个问题：OCR 匹配脆弱 + 全屏截图干扰。

**问题根因**：Step 5 对全屏截图做 OCR 搜索"发送"，但转发弹窗的搜索框里也有"发送"字样（发送给 xxx），导致点击位置错误（点到了搜索框而不是发送按钮）。

**修复**：Step 3 成功打开转发弹窗后，检测并保存弹窗的窗口 bounds（`x, y, w, h`）。Step 5 先裁剪该弹窗区域再 OCR 识别（只识别弹窗内的文字），避免搜索框干扰。如果裁剪区域识别失败才回退到全屏识别。

日志标识：`[弹窗裁剪] 700x540 区域识别` → `找到发送按钮: (788, 232)`。

**失败时截图备份**：仍然保留，备份为 `/tmp/send_button_FAILED_YYYYMMDD_HHMMSS.png`。
