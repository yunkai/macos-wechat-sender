...
**真实根因**（用失败截图 + 日志交叉验证）：
- `CGWindowListCopyWindowInfo` 返回所有微信窗口，列表顺序不固定
- 微信**主窗口**在小窗口模式下是 `808×533`，刚好满足弹窗检测条件 `300 < w < 1200, 200 < h < 700`
- 主窗口 `kCGWindowName == "微信"`，转发弹窗 `kCGWindowName != "微信"`（通常为空）
- 当主窗口在列表中排在弹窗前面时，`break` 将其误选为弹窗 bounds
- 裁剪主窗口区域 → 不包含右下角「发送」按钮 → 失败

**验证**（用失败截图模拟二选一）：
```
主窗口 (273,88,808,533): ❌ 没有「发送」
弹窗   (406,168,700,540): ✅ 找到发送 (1013,694)
```

**修复**
- `popup_bounds` 检测改为收集所有候选窗口，**按 `kCGWindowName` 过滤**：排除 `name == "微信"` 的主窗口，只选弹窗
- 提取 `_find_send_in_ocr()` 辅助函数，消除遍历逻辑重复
- 不做全屏回退兜底——第一道防线正确就够

### v1.20.0（2026-04-26）
**Bug 修复**
- **夜间模式卡片定位失败**：`find_card_center()` 正确检测到分割线和卡片文字，但计算的右击位置在分割线下方（`line_y + 15`），而非卡片内容区域。分割线只是验证卡片存在的标志，右击应点击到文字位置（`cy`）。
- **新增分割线宽度过滤**：最小 40px（排除极小 UI 装饰元素）、最大窗口宽度 50%（排除全屏输入框分隔线）。
- **新增 known_title 参数**：`find_card_center()` 支持传入已知文章标题，优先精确匹配卡片位置。

**根因**：
1. 卡片中心计算错误——`card_cy = line_y + 15` 落在分割线下方（输入框区域）。正确做法：`card_cy = cy`（文字位置在卡片内部）。
2. 文章卡片的标题文字 OCR 无法识别（富媒体渲染，非纯文本），只能识别到底部的来源名（如「正法文集」）。

**经验教训**：
- 分割线只用于**验证**卡片结构，真正的右击位置在**文字坐标**（`cy`），而非分割线下方
- 卡片的来源名（底部小字）是 OCR 最稳定能检测到的文本
- 不能依赖 URL 位置推算（via_contact ≠ target_contact 时目标聊天无 URL）
- 输入框分隔线是反例——线宽横跨全窗口，容易误判为卡片分割线

### v1.19.0（2026-04-26）
**Bug 修复**
- **Step 4 右键点击到输入框而非卡片**：`find_card_center()` 误判输入框为文章卡片，导致 Step 4 右键点击弹出的是输入框右键菜单（而非卡片右键菜单），OCR 找不到「引用」选项

**根因**：`find_card_center()` 用"白色/深色背景 + 下方灰色/浅色分割线"检测文章卡片，但输入框顶部边框（分隔聊天区域和输入框的那条线）视觉特征相似，被误判为卡片的底部分割线。检测到的卡片中心 `(761, 515)` 实际落在输入框范围内（截图内 y=414，接近窗口底部 472）。

**修复方向**：在 `find_card_center()` 中加 y 坐标上限检查——卡片中心不能太靠近窗口底部（`screen_y < wy + wh - 120`），超过此阈值则放弃该候选，尝试其他候选。

### v1.18.0（2026-04-21）
**Bug 修复**
- **Step 2 链接点击错位**：全屏 OCR 误识聊天区域外 URL（搜索框/输入法候选），新增聊天区域过滤逻辑，排除 `scan_y_top`（标题栏+搜索栏）以上的 URL
- **Step 5 发送按钮点击到"发送给"标题**：模糊匹配 `"发送" in text` 被转发弹窗标题"发送给 xxx"误中，改为精确匹配 `t == "发送" or t == "发送 "`
- **Step 5 截图干扰**：全屏截图含搜索框"发送"字样，改为优先裁剪弹窗区域再 OCR，失败才回退全屏

**测试**：回归测试 3a/3b（大窗口 1200×700）全部通过

### v1.17.0（2026-04-20）
**Bug 修复**
- **Step 3 fallback 移除**：移除 `↓ + Enter` 盲操作，改为失败时截图备份 + 打印 OCR 所有文字后 fail-fast
- **Step 2 URL 过滤**：过滤聊天区域外的 URL，避免点到搜索框/输入法候选
...
**模式 1：popup_bounds 误选主窗口**（v1.21.0 修复）
`CGWindowListCopyWindowInfo` 窗口列表顺序不固定，主窗口（小窗口模式 808×533）满足弹窗尺寸条件时可能被误选。修复：增加 `kCGWindowName` 过滤，排除名为「微信」的主窗口。

**排查**：查看备份截图 `/tmp/send_button_FAILED_*.png`。如果全屏截图明显有「发送」按钮但运行时报失败 → 模式 1。

**模式 2：全屏截图干扰**（v1.18.0 修复）
全屏截图含搜索框"发送给 xxx"字样，模糊匹配 `"发送" in text` 误命中。修复：精确匹配 `t == "发送" or t == "发送 "`，并优先裁剪弹窗区域 OCR 避免搜索框干扰。

**日志标识**：
- 弹窗检测成功：`[弹窗] 位置(406,168) 大小700x540` → `找到发送按钮: (1013,694)`
...
**根因**：输入框顶部有一条分隔线（分隔聊天区域和输入框），视觉特征（白色背景 + 下方浅色/灰色直线）与文章卡片非常相似，被 `find_card_center()` 误判为卡片。检测到的"卡片中心"落在输入框范围内，右键点击后弹出的是输入框右键菜单而非卡片右键菜单。

**排查方法**：查看 cron 输出中的日志
```
[卡片检测] 卡片中心(屏幕): (761, 515)
[菜单窗口] 位置(737,491) 大小210x127
```
菜单窗口只有 210x127 px，是典型的输入框右键菜单大小（卡片右键菜单通常更大，包含「转定向」「提醒」等更多选项）。

**修复方向**：在 `find_card_center()` 的候选排序后，加 y 坐标上限检查——卡片中心 `screen_y < wy + wh - 120` 才视为有效，否则放弃该候选并尝试其他候选。
